...
class StructureArticle():
    ...
    def get_main_idea(self,task_name, character_a, knowledge_content="None", language="Chinese"):
        if self.api_type=="yiyan":
            ...
            这里有一个任务”{task_name}“,请你基于我给你的参考内容针对该任务生成该任务类型文章的中心主题,如果参考内容无信息参考，则生成主题符合任务即可。要有创造力和想象力。回复字数在50字以内。\n\n参考内容：\n{knowledge_content}\n\n请你生成的主题更具体。要有创造力和想象力。回复字数在50字以内。不要添加任何其他内容。
            """
    ...
    def get_title_name(self,main_idea,language="Chinese"):
        if self.api_type=="yiyan":
            ...
            # """
            messages = f"""你是一个”PPT制作专家“，你可以基于具体的中心主题提炼出对应的标题。无论输入语言是什么，您都必须用{language}输出文本。
                       这里有一个中心主题”{main_idea}“,请你基于中心主题提炼出对应的标题,要求回复字数在10字以内。不要添加任何其他内容。
                       """
            yiyan_access_token = yiyan.get_access_token()
            out_str = yiyan.yiyan_api(messages,yiyan_access_token)
        return out_str
    def get_first_sentence(self,task_name, main_idea, character_a, knowledge_content="None", language="Chinese"):
        ...
        #     """
        if self.api_type == "yiyan":
            messages = f"""你是一个善于写作的助手，能够根据信息创造精炼和有深度的内容。请你基于我给你的参考内容根据我给你布置的任务和中心主题，进行文章第一段的内容创作，如果参考内容无信息参考，则生成主题符合任务即可。\n\n参考内容：\n{knowledge_content}\n\n。无论输入语言是什么，您都必须用{language}输出文本。\n输出要求如下:\n
                1.  要求展示在该领域的专业知识。
                2.  要求内容丰富，引导全文。
                3.  字数要求200字左右
                请基于任务”{task_name}“和中心主题”{main_idea}“进行文章第一段的创作.
    ...
    def get_muti_sub_idea(self,task_name, main_idea, character_a, first_sentence, knowledge_content="None",
        ...
        if self.api_type=="yiyan":
            ...
            # """
            messages = f"""你是一个善于写作的助手，能够根据参考信息创造精炼和有深度的内容。请你基于我给你的参考内容根据我给你的任务名称，中心主题，第一段内容，将中心主题分解为多个子主题，要求按照逻辑顺序先后排列。\n\n参考内容：\n{knowledge_content}\n\n。无论输入语言是什么，您都必须用{language}输出文本。
                  任务名称为“{task_name}”，中心主题为“{main_idea}”,文章的第一个段落内容为“{first_sentence}”。请你将中心主题分解为多个子主题，作为文章的观点。要求内容简洁.每个子主题字数少于15字。格式为:\n1. \n2. 
                  """
    ...
    def get_sub_content(self,task_name, main_idea, sub_idea, character_a, knowledge_content="None", language="Chinese"):
        if self.api_type == "yiyan":
            ...
            messages = f"""：你是一个善于写作的助手，能够根据参考信息创造精炼和有深度的内容，并且可以按照规则格式输出内容。请你基于我给你的参考内容，围绕给你的子主题来拆解多个要点。无论输入语言是什么，您都必须用{language}输出文本。
                              _______________________
                              参考内容：\n\n
                              任务名称为{task_name}，中心主题为{main_idea}，子主题为{sub_idea}
                              ————————————————————————
                              \n\n规则格式：
                              ...
                              ———————————————————————————
                             输出样例：
                             1:人工智能未来三年会更加受关注[image]https://aaaaa.gczx.com/dsfsdf.png

                             2:人工智能相关应用明年会迎爆发[image]https://aaaaa.gczx.com/dsfsdfdfsddfdf.png
                              """
