...
class Tree2PPT:
    ...
    def init_pptx(self,theme_path = "../my_ppt_mode/3"):
        prs = Presentation()
    ...
    def traverse_tree(self, heading):
        ...
        if heading is not None and (heading.source is None or heading.source.strip() == ''):#当主题下面是分主题时
        ...
        elif heading is not None and len(heading.source.split("\n\n"))==1:
            ...
            for content_i in range(len(input_text_list)):
                if content_i == 0:
                    ...
                    if self.theme_param["main_page"].get("img_info") and len(self.img_dicts) > 0:
                        ...
                        img_dict = {}
                    MD2Slide(self.prs, self.theme, heading.text, img_dict=img_dict,
                             content=input_text_list[content_i].strip())
                else:
                    if self.theme_param["main_page"].get("img_info") and len(self.img_dicts) > 0:
                        img_dict = self.img_dicts[random.randint(0, len(self.img_dicts) - 1)]
                    else:
            ...
            """
            原来的逻辑就是，如果子主题生成的过长，或者分段了，就拆分成若干段，写若干页。
            那么改为：子主题不要写在3页，而是一页写3个part
            """
            if self.theme_param["main_page"].get("img_info") and len(self.img_dicts) > 0:
                img_dict = self.img_dicts[random.randint(0, len(self.img_dicts) - 1)]
            else:
                img_dict = {}
            img_list = ['test_img/图3.jpg','test_img/图4.jpg','test_img/图5.jpg','test_img/图1.png','test_img/图2.png','test_img/图6.png']
            sampled_images = random.sample(img_list, 3)
            Part2Slide(self.prs, self.theme, heading.text, img_list=sampled_images, content_list=content_list)
...
class Part2Slide:
    ...
    def __init__(self, presentation, theme_path, title, content_list, *args,img_list=[], **kwargs):
        self.presentation = presentation
        self.slide = presentation.slides.add_slide(presentation.slide_layouts[6])
        self.title = title
        ...
        with open(self.theme_param_path,encoding="utf-8") as f:
            self.theme_param = json.load(f)
        print(self.theme)
        page_params = self.theme_param["main_part_page"]

        self.part_content_boxs = [(Cm(p['part_info'+str(i)]["pos_x"]),Cm(p['part_info'+str(i)]["pos_y"]), Cm(p['part_info'+str(i)]["width"]),Cm(p['part_info'+str(i)]["height"])) for p,i in zip(page_params['content_infos'],range(len(page_params['content_infos'])))]
        ...
        self.img_boxs = [(Cm(p['img_info'+str(i)]["pos_x"]),Cm(p['img_info'+str(i)]["pos_y"]), Cm(p['img_info'+str(i)]["width"]),Cm(p['img_info'+str(i)]["height"])) for p,i in zip(page_params['img_infos'],range(len(page_params['img_infos'])))]
        self.img_theme= theme_path+"/"+"img"
        self.init_font(**kwargs)
        self.init_slide()
        self.init_title()
    ...
    def init_content(self):
        ...
        for i in range(len(self.content_list)):
            content_box = self.part_content_boxs[i]
            text_box_content = shapes.add_textbox(*content_box)
            tf =  text_box_content.text_frame
            tf.clear()
            tf.vertical_anchor = MSO_VERTICAL_ANCHOR.TOP
            tf.word_wrap = True
            ...
            content_splits = content.split('[image]')
            if len(content_splits)>1:
                url = content_splits[1]
                try:
                    response = req.get(url.strip())
                    image = Image.open(BytesIO(response.content))
                    image.save("temp_img.png")
                    picture = self.slide.shapes.add_picture("temp_img.png", *self.img_boxs[i])
                except Exception as e:
                    print('error url')

            content_text = content_splits[0].strip()
            paragraph.text = content_text

            self.processing_md_str(content.replace("<p>", "").replace("</p>", "\n"))
            # TODO 处理正文
            self.get_font(paragraph.font, MarkdownCategory.CONTENT)
            paragraph.vertical_anchor = MSO_VERTICAL_ANCHOR.TOP
...
class THEME_MD2Slide:
    ...
    def __init__(self, presentation, theme_path, title, *args, **kwargs):
        ...
        if first_page_params["title_info"].get('font_title_color'):
            c =  first_page_params["title_info"].get('font_title_color')
            self.font_title_color = RGBColor(c[0],c[1],c[2])
        self.img_theme= theme_path+"/"+"img"
        self.init_font(**kwargs)
        self.init_slide()
        self.init_title()
        if first_page_params.get("time_info"):
            self.time_box =(Cm(first_page_params["time_info"]["pos_x"]),Cm(first_page_params["time_info"]["pos_y"]),Cm(first_page_params["time_info"]["width"]),Cm(first_page_params["time_info"]["height"]))
            self.init_time()
        if first_page_params.get("name_info"):
            self.name_box =(Cm(first_page_params["name_info"]["pos_x"]),Cm(first_page_params["name_info"]["pos_y"]),Cm(first_page_params["name_info"]["width"]),Cm(first_page_params["name_info"]["height"]))
            self.init_name()
        #预留的判断是否有副标题的位置0
        # if first_page_params.get("second_title_info") and True:
        #     self.second_title_box = (Cm(first_page_params["second_title_info"]["pos_x"]), Cm(first_page_params["second_title_info"]["pos_y"]),
        #                      Cm(first_page_params["second_title_info"]["width"]), Cm(first_page_params["second_title_info"]["height"]))
    ...
    def init_time(self):
        shapes = self.slide.shapes
        text_box = shapes.add_textbox(*self.time_box)
        tf = text_box.text_frame
        tf.clear()  # Clear existing content
        tf.word_wrap = True  # 允许自动换行
        # 添加标题
        ...
        paragraph.text = formatted_date

        paragraph.font.size = Pt(self.theme_param["first_page"]['time_info']['font_size'])
        paragraph.font.color.rgb = RGBColor(255,255,255)
        paragraph.word_wrap = True
    def init_name(self):
        shapes = self.slide.shapes
        text_box = shapes.add_textbox(*self.name_box)
        tf = text_box.text_frame
        tf.clear()  # Clear existing content
        tf.word_wrap = True  # 允许自动换行
        # 添加标题
        ...
        paragraph.font.size = Pt(self.theme_param["first_page"]['name_info']['font_size'])
        paragraph.font.color.rgb = RGBColor(255,255,255)
        paragraph.word_wrap = True

    def init_second_title(self):
        shapes = self.slide.shapes
        text_box = shapes.add_textbox(*self.second_title_box)
        tf = text_box.text_frame
        tf.clear()  # Clear existing content
        # tf.auto_size = MSO_AUTO_SIZE.SHAPE_TO_FIT_TEXT
        # tf.vertical_anchor = MSO_VERTICAL_ANCHOR.TOP
        ...
        # 添加标题
        paragraph = tf.paragraphs[0]
        paragraph.text = '————这是关于副标题的测试'
        paragraph.font.size = Pt(self.theme_param["first_page"]['second_title_info']['font_size'])
        paragraph.font.color.rgb = RGBColor(255, 255, 255)
        # self.get_font(paragraph.font, MarkdownCategory.TITLE)
        paragraph.word_wrap = True
